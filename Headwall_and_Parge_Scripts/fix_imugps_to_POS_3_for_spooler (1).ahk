...
loop % timeOrig.total
{
	...
	{
		If (gtDecRate > 1000 AND gtDecRate > 0)
		{
...
{
	igTest	:= imugpsPOS.TimeH[a_index] . imugpsPOS.TimeM[a_index] . imugpsPOS.TimeS[a_index] . imugpsPOS.TimeDec[a_index], igTest += 0
	
	If (abs(tofStart-igTest) <= 1 AND igStartindex = "")
	{
		igStartindex := A_Index
		igStartlog := A_Index
	}
	Else If (abs(tofEnd-igTest) <= 1 AND igEndindex = "")
	{
		igEndindex := A_Index
	}
}
...
imugpsOut.close()
progress, 100, DONE!
msgbox, New imu_gps.txt output here: %Dir% `r`n Old imu_gps.txt copied here: %Dir%\FILE_ORIGINALS\
