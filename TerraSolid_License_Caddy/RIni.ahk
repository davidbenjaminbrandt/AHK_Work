...
;*************************************************************************************************************
;Example 3: how to build an ini from scratch using the No_E functions.

;First we start by calling the ini_create() function.

Ini_Create(3,1)

;Paramater 1 is the reference word/variable you will use to modify that ini.
;It can be a number or variable or word. Any value is valid as long as that value can be used as a variable name.
;Paramater 2 is to set "correct_errors" to 1 or 0 depending on what you pass it.
;By default it is set to 1 so it is not needed unless you wish to disable it.
;If it is set to 1, when you call the High/Low E functions they will attempt to correct basic errors such as sections/keys not existing.


;Now we will add a section to the ini.
;If you are going to use the High E functions you do not NEED to add the sections manually because they will automatically create sections/keys as you attempt to use them if you created the ini with "correct_errors" set to 1/true.
;However, for this example we will be using the No_E function so we must add the sections manually.

RIni_AddSection_NE(3,"Section_1")

;Paramater 1 is the reference word/variable for the ini.
;Paramater 2 is the section you wish to add.
;Note: Spaces in section/key names are not allowed with the No_E functions.


;Now we will add a comment to the section.

RIni_SetSectionComment_NE(3,"Section_1","This is section_1. It was created with the No_E section function.")

;Paramater 1 is the reference word/variable for the ini.
;Paramater 2 is the section you wish to set the comment on.
;Paramater 3 is the comment you wish to set.
;Note: You do not need to add the comment character (;) to the comment. It is added automatically.
;Note: Spaces in section/key names are not allowed with the No_E functions.


;Now we will add a key to the section.

RIni_AddKey_NE(3,"Section_1","Key_1")

;Paramater 1 is the reference word/variable for the ini.
;Paramater 2 is the section you wish to add the key to.
;Paramater 3 is the key name you wish to add.
;Note: Spaces in section/key names are not allowed with the No_E functions.


;Now we will add a comment to key_1.


RIni_SetKeyComment_NE(3,"Section_1","Key_1","This is the comment for section_1 key_1. It was added with the No_E function.")

;Paramater 1 is the reference word/variable for the ini.
;Paramater 2 is the section that contains the key you want to add the comment to.
;Paramater 3 is the key that you want to add the comment to.
;Paramater 4 is the comment you wish to add.
;Note: You do not need to add the comment character (;) to the comment. It is added automatically.
;Note: Spaces in section/key names are not allowed with the No_E functions.


;Now we will set the value of Key_1.

RIni_SetKeyValue_NE(3,"Section_1","Key_1","My value for key_1 which is in section_1.")

;Paramater 1 is the reference word/variable for the ini.
;Paramater 2 is the section that contains the key you want to set the value of.
;Paramater 3 is the key that you want to set the value of.
;Paramater 4 is the value you want to set the key to.
;Note: Spaces in section/key names are not allowed with the No_E functions.


;Now we can either get/save/manipulate the sections/keys/comments.
;For now we are going to save the ini to the location of the script.

RIni_Write_LE(3,A_ScriptDir "\My New INI.ini","`r`n")
